...
//	Metodo para acceder al valor de la columna 1 fila 1:
	public Object SelectEscalar(String sel) {
		...
		try {
			...
			rset.next(); // Como los objetos statement se generan en la fila 0, para acceder a la primera
							// se ejecuta dicho metodo.
			res = rset.getObject(1); // Obtiene el valor de la columna pasada por argumento, en la actual fila (1),
										// tras haber ejecutado el metodo next
...
//	Metodo para hacer guardar una lista con toda la operacion del SELECT:
	public List<Object[]> Select(String sel) {
		...
		try {
			...
			int numCol = meta.getColumnCount(); // Total de columnas (las que hayamos seleccionado) que tiene la
												// busqueda SELECT
			while (rset.next()) {
				Object[] tupla = new Object[numCol];
				for (int i = 0; i < numCol; ++i) {
					tupla[i] = rset.getObject(i + 1);
				} // Crea una tupla con toda la fila del SELECT, agrupando los datos que
					// pertenecen al mismo objeto
				lista.add(tupla); // A�ade a la coleccion de datos seleccionados por el SELECT, tantos como
									// filas
			}
			rset.close(); // Cerramos recursos
			stmt.close(); // Cerramos recursos
...
//	Operacion INSERT para la BDD:
	public void Insert(String ins) {
		...
		try {
			...
			Statement stmt = con.createStatement(); // Crea a partir de la conexion un objeto Statement que se
													// enlazar� a la BDD
			stmt.executeUpdate(ins); // Con dicha operacion en este objeto ejecutamos una instruccion SQL, el
										// argumento que le entra es la propia instruccion SQL
...
//	Operacion DELETE para la BDD:
	public void Delete(String del) {
		...
		try {
			...
			Statement stmt = con.createStatement(); // Crea a partir de la conexion un objeto Statement que se
													// enlazar� a la BDD
			stmt.executeUpdate(del); // Con dicha operacion en este objeto ejecutamos una instruccion SQL, el
										// argumento que le entra es la propia instruccion SQL
			stmt.close(); // Cerramos los recursos
		} catch (SQLException e) {
...
//	Operacion UPDATE para la BDD:
	public void Update(String up) {
		...
		try {
			...
			Statement stmt = con.createStatement(); // Crea a partir de la conexion un objeto Statement que se
													// enlazar� a la BDD
			stmt.executeUpdate(up); // Con dicha operacion en este objeto ejecutamos una instruccion SQL, el
									// argumento que le entra es la propia instruccion SQL
			stmt.close(); // Cerramos los recursos
		} catch (SQLException e) {
